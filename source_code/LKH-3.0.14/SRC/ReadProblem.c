 ...
 *              problem
 * PC-TSP       Data for a prize-collecting traveling salesman tour problem
 ...
 * TOP          Data for a team orienteering problem
 ...
 *
 * NODE_SCORE_SECTION :
 * The scores of all nodes of an OP, PTP, PC-TSP, or TOP are given in the form
 * (per line)
 *
 *    <integer> <integer>
 *
 ...
 * 
 * NODE_PENALTY_SECTION :
 * The penalties of all nodes of a PCTSP are given in the form (per line)
 *
 *    <integer> <integer>
 *
 * The first integer specifies a node number, the second its penalty.
 *
 * 
...
static void Read_NODE_PENALTY_SECTION(void);
...
void ReadProblem(void)
{
    ...
    while ((Line = ReadLine(ProblemFile))) {
        ...
        else if (!strcmp(Keyword, "NODE_PENALTY_SECTION"))
            Read_NODE_PENALTY_SECTION();
    ...
    else if (ProblemType == PC_TSP)
        Penalty = Penalty_PC_TSP;
    ...
    if (ProblemType != CVRP && ProblemType != CVRPTW &&
        ...
        ProblemType != PCTSP && ProblemType != OP && ProblemType != TOP) {
        M = INT_MAX / 2 / Precision;
        for (i = Dim + 1; i <= DimensionSaved; i++) {
    ...
    if (ProblemType == MSCTSP || ProblemType == OP || ProblemType == TOP ||
        ProblemType == PTP)
        Optimum *= -1;
    fclose(ProblemFile);
...
static void Read_NODE_PENALTY_SECTION(void)
{
    ...
    for (i = 1; i <= Dim; i++) {
        fscanint(ProblemFile, &Id);
        if (Id <= 0 || Id > Dim)
            eprintf("NODE_PENALTY_SECTION: Node number out of range: %d", Id);
        N = &NodeSet[Id];
        if (!fscanint(ProblemFile, &N->Penalty))
            eprintf("NODE_PENALTY_SECTION: Missing score for node %d",
                    Id);
    }
}
