 ...
 * Default: DBL_MAX
 *
 * TOP_SOLUTION_FILE = <string>
 * Specifies the name of a file where the solution of a TOP instance
 * is to be written.
 * The character $ in the name has a special meaning. All occurrences are
 * replaced by the cost of the tour.
 *
...
void ReadParameters(void)
{
    ...
    while ((Line = ReadLine(ParameterFile))) {
        ...
        } else if (!strcmp(Keyword, "TOP_SOLUTION_FILE")) {
            if (!(TOPSolutionFileName = GetFileName(0)))
                eprintf("TOP_SOLUTION_FILE: string expected");
